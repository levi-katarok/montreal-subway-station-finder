...
export function ChatInterface({ onRouteRequest, messages, setMessages, isLoading: externalIsLoading, language }: ChatInterfaceProps) {
  ...
  const [pendingRouteAction, setPendingRouteAction] = useState<RouteAction | null>(null);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Check if route was mentioned and store action for button
  ...
  return (
    <div className="bg-black flex flex-col h-full max-h-screen">
            ...
            <div className="bg-white/10 border border-white/20 rounded-2xl px-4 py-3">
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm text-white/80">
                  {language === 'en' ? 'Analyzing route...' : 'Analyse de l\'itinéraire...'}
                </span>
              </div>
            </div>
          </div>
      ...
      {/* Show Route Button */}
      {pendingRouteAction && (
        <div className="fixed bottom-32 left-0 right-0 px-4 z-50 pointer-events-none">
          <div className="max-w-md mx-auto pointer-events-auto">
            <button
                ...
                if (onRouteRequest) {
                  ...
                  try {
                    await onRouteRequest(
                      pendingRouteAction.query,
                      pendingRouteAction.from,
                      pendingRouteAction.to
                    );
              ...
              className="w-full px-6 py-4 bg-white hover:bg-white/90 text-black rounded-2xl font-bold shadow-2xl border-2 border-white/20 transition-all transform hover:scale-105 flex items-center justify-center gap-3"
            >
              <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 20l-5.447-2.724A1 1 0 013 16.382V5.618a1 1 0 011.447-.894L9 7m0 13l6-3m-6 3V7m6 10l4.553 2.276A1 1 0 0021 18.382V7.618a1 1 0 00-.553-.894L15 4m0 13V4m0 0L9 7" />
              </svg>
              <span>{language === 'en' ? 'Show Route on Map' : 'Afficher l\'itinéraire'}</span>
            </button>
          </div>
        </div>
      )}
